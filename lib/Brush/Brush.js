...
whiteboard.Brush = (function(_){
  ...
  Brush = function(template, name){
    ...
    /**
     * Expose brush specific onpaint functions
     * Onpaint functions specific to this brush will be called
     * on a stroke any time this brush paints a stroke
     */
     this.onpaint = {};
  ...
  */
  ...
  /**
   * Expose universal onpaint functions.
   * Each function inside here will be called on every stroke 
   * that is painted when it is painted
   *
   * Each onpaint will be passed the stroke as a parameter.
   * @param {Stroke}
   *
   * Example:
   *   whiteboard.Brush.onpaint.enableJqueryDraggable = function(stroke) {
   *      $(stroke).draggable({handle: '.drag-handle'});
   *   };
   */
  Brush.onpaint = {};
